...
for x in games:
    if x['homeScore'] > x['awayScore']:
        ...
        wcheck = teams.index(x['home'])
        wcheck = int(wcheck)
        wincheck = teams[wcheck]
        teamData[wincheck]['wins'] += 1
        teamData[wincheck]['gamesPlayed'] += 1

        lcheck = teams.index(x['away'])
        lcheck = int(lcheck)
        ...
        wcheck = int(wcheck)
        wincheck = teams[wcheck]
        teamData[wincheck]['ties'] += 1
        teamData[wincheck]['gamesPlayed'] += 1

        lcheck = teams.index(x['home'])
        lcheck = int(lcheck)
        losecheck = teams[lcheck]
        teamData[losecheck]['ties'] += 1
        teamData[losecheck]['gamesPlayed'] += 1
